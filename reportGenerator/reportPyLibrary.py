...
class Report:
    ...
    def __init__(self, df='', jsonObject='', header='', font='', fill='', border='', alignment='', number_format= 'General', protection = ''):
        if type(jsonObject)==str and jsonObject!='' and type(df)!=str:
            ...
            try:
                ...
                raise Exception("Type mismatch. Requires queryset must be of type of either 'DataFrame' or 'Json Object'")
        ...
        if type(alignment) == str and alignment == '':
            ...
            self.alignment = Alignment(horizontal= alignment.get('alignment') if 'alignment' in alignment else 'general',
                                        ...
                                        text_rotation = alignment.get('text_rotation') if 'text_rotation' in alignment else 0,
                                        wrap_text = alignment.get('wrap_text') if 'wrap_text' in alignment else False,
                                        shrink_to_fit = alignment.get('shrink_to_fit') if 'shrink_to_fit' in alignment else False,
